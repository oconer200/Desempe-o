...
-- 1. EVT_VerificarMantenimientoMaquinaria: Verifica periódicamente la necesidad de mantenimiento en la maquinaria basado en el horómetro.
...
-- 2. EVT_ActualizarEstadoSiembras: Actualiza el estado de las siembras según la etapa de crecimiento del cultivo.
...
DO
  ...
  SET s.Estado = 'Listo para Cosecha'
  WHERE s.Estado = 'En Crecimiento'
    AND CURDATE() >= s.FechaEstCosecha;
    
   

-- 3. EVT_GenerarAleraCaducidadInsumos: Genera alertas para insumos próximos a caducar.
...
DO
  UPDATE Insumos
  SET Alerta = 'PRÓXIMO A VENCER'
  WHERE FechaCaducidad <= DATE_ADD(CURDATE(), INTERVAL 30 DAY) 
    AND FechaCaducidad > CURDATE()
    AND stock > 0;
    
    

-- 4. EVT_CalcularCostosPorParcela: Calcula periódicamente los costos acumulados por parcela.
...
DO
BEGIN
    -- Limpiamos la tabla para el nuevo cálculo
    TRUNCATE TABLE Costos_Parcelas_Acumulados;

    INSERT INTO Costos_Parcelas_Acumulados (id_parcela, costo_insumos, costo_mano_obra, costo_total, ultima_actualizacion)
    SELECT 
        p.id_parcela,
        -- Suma de insumos usados en la parcela (vía movimientos)
        IFNULL((SELECT SUM(m.cantidad * i.precio) 
                FROM Movimientos_Insumos m 
                JOIN Insumos i ON m.id_insumo = i.id_insumo 
                WHERE m.Destino = p.codigo AND m.tipo = 'SALIDA'), 0) AS costo_insumos,
        
        -- Suma de mano de obra (horas * salario promedio, asumiendo salario base/160 horas mes)
        IFNULL((SELECT SUM(a.horas * (e.salario / 160)) 
                FROM Asignaciones a 
                JOIN Empleados e ON a.dni_empleado = e.dni 
                WHERE a.id_parcela = p.id_parcela), 0) AS costo_mano_obra,
                
        0 AS costo_total, -- Se calcula abajo
        NOW()
    FROM Parcelas p;

    -- Actualizamos el total sumando ambos conceptos
    UPDATE Costos_Parcelas_Acumulados SET costo_total = costo_insumos + costo_mano_obra;
END //


-- 5. EVT_RenovarContratoEmpleados: Verifica contratos de empleados próximos a vencer y genera alertas.
...
DO
  UPDATE Empleados
  SET AlertaContrato = 'PRÓXIMO A VENCER'
  WHERE FechaFinContrato <= DATE_ADD(CURDATE(), INTERVAL 15 DAY) 
    AND FechaFinContrato >= CURDATE()
    AND AlertaContrato != 'Finalizado';
